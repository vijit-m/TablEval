...
#VIJIT
###################################
from transformers import RobertaForSequenceClassification, RobertaTokenizer, RobertaConfig, AdamW
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        padding_masks = [0] * (max_seq_length - len(input_ids))
        # VIJIT: Zero-pad for input_ids
        pad_id = tokenizer.convert_tokens_to_ids([PAD])
        padding_input = pad_id*(max_seq_length - len(input_ids))
        padding_segment = [1]*(max_seq_length - len(input_ids))

        input_ids += padding_input
        input_mask += padding_masks
        segment_ids += padding_segment
...
def main():
    ...
    ## Other parameters
    #VIJIT: Make sure here that if we dont pass an argument of a valid model for RoBERTa it will cause problems (because it automatically takes up 'bert-base-multilingual-cased').
    ### I dont see why bert-base-uncased was not used so we will try with 'roberta-base'
    ...
    #VIJIT: Need of Roberta tokenizer here
    ...
    config1=RobertaConfig.from_pretrained('roberta-base',output_hidden_states=True)
    roberta =RobertaModel.from_pretrained('roberta-base',config=config1)
    #model = RobertaForSequenceClassification.from_pretrained(load_dir, num_labels = num_labels)
    ...
    class ROBERTAGRUSentiment(nn.Module):
      def __init__(self,
          ...
          self.linear1 = nn.Linear(512*768, 256)
          self.dropout1 = nn.Dropout(linear1)
      ...
      def forward(self,input_ids, input_mask, label_ids):
          ...
          with torch.no_grad():
              embedded= self.roberta(input_ids, input_mask )[0]
                  
          #print(embedded.size(), "shsjsmms")        
          #embedded = [batch size, sent len, emb dim]
          
          # abc , hidden = self.rnn(embedded)
          ...
          #embedded=embedded.reshape(-1,768 )       
          #hidden = [batch size, hid dim]
          
          embedded=embedded.reshape(input_ids.shape[0], -1)
          ...
          if label_ids is not None:
              if self.output_dim == 1:
                  ...
                  loss = loss_fct(logits.view(-1, self.output_dim), label_ids.view(-1))
    ...
    model = ROBERTAGRUSentiment(roberta,
                            HIDDEN_DIM,
                            OUTPUT_DIM,
                            N_LAYERS,
                            BIDIRECTIONAL,
    ...
    if args.do_train:
        ...
        if args.fp16:
            ...
            #VIJIT: Changed optimizer to AdamW here
            optimizer = AdamW(model.parameters(),
                                 lr=args.learning_rate, correct_bias=True
                                )
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.period == 0:
                    ...
                    #output_config_file = os.path.join(output_dir, config1)
